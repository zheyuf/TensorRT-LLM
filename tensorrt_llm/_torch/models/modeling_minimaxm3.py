    ...
    def forward(
        ...
        **kwargs,
        ...
        if spec_metadata is not None and spec_metadata.is_layer_capture(self.layer_idx):
            # Eagle3 captures the raw layer output (hidden + residual), so the
            # feed-forward AllReduce must stay in the module and the boundary
            # norm must run unfused for the captured pair to be fully reduced
            # (mirrors DeepSeek-V3's POST-fusion opt-out on capture layers).
