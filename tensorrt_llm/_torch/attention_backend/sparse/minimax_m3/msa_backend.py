...
class MiniMaxM3MsaSparseAttentionMetadata(TrtllmAttentionMetadata):
    ...
    def _build_decode_plans(self) -> None:
        ...
        # The sparse decode planner packs every query token into one wave and
        # asserts total_q * num_q_heads <= 65536. Spec verify multiplies decode
        # tokens by (1 + draft_len), so a large eager batch can exceed that
        # budget; route such batches through the prefill planner, which tiles
        # without the packed-wave cap. Graph capture keeps the decode planner:
        # captured batch sizes are bounded by cuda_graph_config and the decode
        # plan is the capture-safe one.
        total_q = int(qo_lens_cpu.sum().item())
        gqa_kernel_mode = ("prefill" if not capture_graph
                           and total_q * num_q_heads > 65536 else "auto")
        gqa_plan = fmha_sm100.fmha_sm100_plan(
            ...
            num_kv_splits=1,
            causal=True,
            sparse_kernel_mode=gqa_kernel_mode,
        )
