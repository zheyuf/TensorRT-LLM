    ...
    def add_dummy_requests(
        ...
        for i, req_id in enumerate(request_ids):
            ...
            if prepare_resource:
                ...
                draft_kv_cache = None
                if draft_kv_cache_manager is not None:
                    # A V2 target always gets a V2 draft manager (forced at
                    # creation in _util.py); V1 drafts have no per-request
                    # cache handles and no suspend/resume for the scheduler.
                    assert isinstance(draft_kv_cache_manager, KVCacheManagerV2)
            ...
            if is_gen:
                ...
                if prepare_resource:
                    ...
                    if not success:
                        release_resources(
                            req,
                            free_draft_resources=draft_kv_cache is not None,
                        )
                        return None
