...
class KvCacheCreator:
    ...
    def _get_draft_kv_cache_manager_cls(self, effective_draft_config,
        ...
        A V2 target requires a V2 draft: the V2 scheduler suspends/resumes
        the draft manager through V2-only APIs, and V2's add_dummy_requests
        allocates the draft through per-request cache handles. Targets that
        are structurally V2 (e.g. sparse-attention managers) don't set
        use_kv_cache_manager_v2, so a plain-transformer draft (Eagle3,
        external drafter) would otherwise resolve to V1 here.
        """
