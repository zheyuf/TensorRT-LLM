...
"""Pure-logic tests for MiniMaxM3DraftKVCacheView and the shared draft layout."""
...
DRAFT_LAYER = 60
SCALE = 178  # P128 sub-pages per M3 mega-slot
ADDR = 0x7000_0000
...
def test_view_geometry():
    view = _make_view()
    # Geometry the attention op reads from the draft-side manager.
    ...
    # Everything else (the cache dtype included) is the shared manager's.
    assert view.dtype == DataType.FP8


def test_block_table_uses_the_views_own_pool_geometry(monkeypatch):
    view = _make_view()
    calls = []
    ...
    def fake_copy(
        ...
        # Unbound call: the V2 implementation must see the view's geometry,
        # not the shared manager's.
...
def test_manager_accessor_builds_and_caches_view():
    ...
    manager_no_draft = _FakeSharedManager()
    manager_no_draft._shared_draft_layer_ids = []
    assert get_view(manager_no_draft) is None


def test_dummy_requests_drop_the_view(monkeypatch):
    seen = {}

    def fake_add_dummy_requests(self, *args, **kwargs):
        seen.update(kwargs)
        return "delegated"

    monkeypatch.setattr(KVCacheManagerV2, "add_dummy_requests", fake_add_dummy_requests)
    manager = MiniMaxM3KVCacheManagerV2.__new__(MiniMaxM3KVCacheManagerV2)

    # The shared manager's own view owns no blocks, so it must not receive
    # duplicate dummy allocations.
    result = MiniMaxM3KVCacheManagerV2.add_dummy_requests(
        manager, [1, 2], draft_kv_cache_manager=_make_view()
    )
    assert result == "delegated"
    assert seen == {"draft_kv_cache_manager": None}

    # A genuinely separate draft manager still goes through.
    seen.clear()
    separate = object()
    MiniMaxM3KVCacheManagerV2.add_dummy_requests(manager, [1], draft_kv_cache_manager=separate)
    assert seen == {"draft_kv_cache_manager": separate}
