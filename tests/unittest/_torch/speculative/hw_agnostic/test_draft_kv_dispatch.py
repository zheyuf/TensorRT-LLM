...
"""Dispatch tests for one-model draft KV cache manager resolution."""
...
class _PlainSharedTarget:
    """Same-geometry shared drafter: no view, the drafter uses the manager."""
...
def test_two_model_modes_resolve_to_none():
    two_model = SimpleNamespace(spec_dec_mode=SimpleNamespace(use_one_engine=lambda: False))
    resources = _FakeResources({ResourceManagerType.KV_CACHE_MANAGER: _SharedTargetWithView()})
    assert get_draft_kv_cache_manager(two_model, resources) is None
...
def test_broken_view_construction_propagates():
    # A failure inside view construction must surface, not silently downgrade
    # to "no view": getattr fetches the bound method without executing it, so
    # the exception escapes from the call itself.
